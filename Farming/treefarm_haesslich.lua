...
if #tArgs >= 1 then -- no error check
    ...
    if tonumber(tArgs[1]) ~= nil then maxCounter = tonumber(tArgs[1]) end
end
...
local function sd(n)
    if n == nil then
        ...
        for i = 1, n do turtle.suckDown() end
    end
end
    ...
    if tmpFuel < minFuel then
        ...
        while tmpFuel < minFuel do
            ...
            if tmpFuel2 ~= tmpFuel then -- tmpItems~=turtle.getItemCount(slotFuel) then
                ...
                print("Need more fuel (" .. tmpFuel .. "/" .. minFuel ..
                          ") in slot " .. slotFuel)
            end
...
local function replantStub()
    ...
    if turtle.compare() then
        ...
        ss(1)
        if pf() then
            actRandomCheckSapling = actRandomCheckSapling +
                                        cIncreaseCheckSapling_Stub
        else
            print("    failure!")
        end
    else
...
local function randomReplant()
    ...
    if turtle.getItemCount(1) > 10 then
        ...
        actRandomCheckSapling = math.max(actRandomCheckSapling - 0.01,
                                         minRandomCheckSapling)
        actRandomCheckSapling =
            math.min(actRandomCheckSapling, cMaxCheckSapling)
...
local function craftFuel()
    ...
    -- step 1 need fuel?
    if (turtle.getFuelLevel() < cMinFuel) and
        (turtle.getItemCount(cSlotChest) == 1) then
        ...
        if tmpFuelItems > 1 then
            ...
            ss(cSlotChest)
            while not turtle.placeUp() do du() end
            ...
            for i = 1, 15, 1 do
                ...
                if i ~= 2 then
                    ...
                    Du(
                        math.max(1,
                                 turtle.getItemCount(2) - cCraftRefuelMaxItems)) -- to keep the wood
                end
            end
            ...
            print(
                "New fuel level (" .. turtle.getFuelLevel() .. "/" .. cMinFuel ..
                    ")")
...
repeat
    ...
    if blnAutoSetup then
        if turtle.getItemCount(3) ~= 1 or turtle.getItemCount(4) < 47 or
            turtle.getItemCount(5) < 8 then
...
until askForInputText(strSimpleCheck) == "" and strSimpleCheck ==
...
repeat
    ...
    if turtle.getItemCount(1) < 11 or turtle.getItemCount(2) == 0 or
        turtle.getItemCount(16) ~= 1 then
        -- inventory not ready
...
while true do
    ...
    print("Starting round ", intCounter,
          " with " .. turtle.getItemCount(1) .. " saplings.")
